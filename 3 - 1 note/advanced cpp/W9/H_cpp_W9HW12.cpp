...
int main()
{
	...
	// (1) 수학 점수를 이용하여 수학과 합격자 3명으로 정하려고 한다.
	// 3명의 아이디를 출력하는 프로그램을 작성하세요.
	printf("(1) 수학 점수를 이용하여 수학과 합격자 3명으로 정하려고 한다.\n3명의 아이디를 출력하는 프로그램을 작성하세요.\n");

	for (int i = 0; i < n - 1; i++) {
		int idx = i;
		for (int j = i + 1; j < n; j++)
			idx = (ps[j].getMath() > ps[idx].getMath()) ? j : idx;
	...
	// (2) 1번을 void display_top3(Person *p, int n) (n은 사람 명수)를 만들어 구현하세요.
	printf("\n(2) 1번을 void display_top3(Person *p, int n) (n은 사람 명수)를 만들어 구현하세요.\n");
	display_top3(ps, n);
	...
	/*
	(4) 사용자가 아이디, 패스워드를 넣어서 기존에 있는 것과 일치되면, “dnfn님 환영합니다.” 처럼 환영의 메시지를 화면에 뿌려준다.
	사용자가 넣은 아이디가 없으면 “입력하신 아이디가 없습니다” 화면에 뿌려주고,
	아이디가 있는데 패스워드가 틀리면 “입력하신 비밀번호가 틀립니다.”를 화면에 뿌려주고,
	3회 비밀번호가 틀린 경우 “3회 비밀번호가 틀렸습니다. 매장으로 방문해주세요”를 화면에 뿌려주기 바랍니다.
...
void display_top3(Person* p, int n)
{
	for (int i = 0; i < n - 1; i++) {
		int idx = i;
		for (int j = i + 1; j < n; j++)
			idx = (p[j].getMath() > p[idx].getMath()) ? j : idx;
